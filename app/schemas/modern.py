...
class LegacyBase(BaseModel):
    ...
    gross_income: float = Field(
        example=75000.50,
        ge=0,
        description="Renda bruta declarada",
    )
    tax_paid: float = Field(
        ...,
        example=15000.75,
        ge=0,
        description="Imposto pago",
    )
    raw_line: str = Field(
        ...,
        example="00001ABCD1234",
        description="Linha original do registro COBOL",
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        example="2025-05-06T14:28:41Z",
